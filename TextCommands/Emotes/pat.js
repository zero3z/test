...
module.exports = {
  ...
  usage: "{prefix}pat <mention | id | tên>",
  ...
  async execute(client, message, args) {
   const response = await axios.get('https://api.waifu.pics/sfw/pat');
    ...
    if (!member) {
      ...
      return client.deleteMsg(message, `${client.e.error} Bạn không thể tự xoa đầu chính mình!`, 5000, "reply");
    }

    const embed = new EmbedBuilder()
    .setAuthor({ name: `${message.author.username} đã xoa đầu ${member.user.username}`, iconURL: message.author.avatarURL({ dynamic: true }) })
