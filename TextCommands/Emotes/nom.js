...
module.exports = {
  ...
  aliases: [],
  cooldown: 3,
  description: "Gặm một ai đó",
  usage: "{prefix}nom <mention | id | tên>",
  ...
  async execute(client, message, args) {
   const response = await axios.get('https://api.waifu.pics/sfw/nom');
    ...
    if (!member) {
      ...
      return client.deleteMsg(message, `${client.e.error} Bạn không thể tự gặm chính mình!`, 5000, "reply");
    }

    const embed = new EmbedBuilder()
    .setAuthor({ name: `${message.author.username} đã gặm ${member.user.username}`, iconURL: message.author.avatarURL({ dynamic: true }) })
